...
export const FrontInscriptionScreen = () => {
  const tags_name = 'miembros';
  ...
  const handleAdd = async (data) => {
    ...
    await createUser(formData).then(response => {
      toast.success('Felicitaciones!! Se ha inscrito correctamente como miembro. Su perfil está siendo revisado para que sea aprobado.');
      navigate('/'+tags_name);
    }).catch(error => {
  ...
  return (
    <div className="card shadow-lg animate__animated animate__fadeIn shadow-sm m-2">
        ...
        <div className="card-body">
            <form onSubmit={handleSubmit(handleAdd)} className='row'>
              ...
              <div className="col-12 d-flex justify-content-end">
                ...
                <Link to={"/"+tags_name} className='btn btn-secondary' >Cancelar</Link>
              </div>
            </form>
        </div>
    </div>
