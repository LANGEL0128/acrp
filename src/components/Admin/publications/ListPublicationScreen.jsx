...
import { deletePublication, listPublication } from '../../../services/publicationService';
...
export const ListPublicationScreen = () => {
    ...
    return (
        <div className="card animate__animated animate__fadeIn shadow-sm">
            ...
            <div className="card-body">
                <Link to="/admin/publicaciones/add" className='btn btn-primary btn-sm'>Agregar</Link>
                <hr />
                                        ...
                                        publications.map((publication, i) => {
                                            return (
                                                <tr key={ publication.uuid }>
                                                    ...
                                                    <td><img src={ publication.photo !== 'null' ? publication.photo : ImgDefault } width={120} height={80} alt="" /></td>
                                                    <td>{ publication.category }</td>
                                                    <td>
                                                        <Link className='btn btn-info btn-sm' to={ '/admin/publicaciones/show/'+publication.uuid }>Ver</Link>
                                                        <Link className='btn btn-warning btn-sm' to={ '/admin/publicaciones/edit/'+publication.uuid }>Editar</Link>
                                                        <button className='btn btn-danger btn-sm' onClick={ () => handleDelete(publication.uuid) }>Eliminar</button>
                                                    </td>
                                                </tr>
                                            );
                                        })
