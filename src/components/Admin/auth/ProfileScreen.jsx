...
export const ProfileScreen = () => {
    ...
    const getUser = async () => {
        setLoading(true);
        await showUser(user.uuid).then(response => {
            setUserx(response.data.data);
        }).catch(error => {
    ...
    const handleEdit = async (data) => {
        ...
        await profile(formData).then(response => {
            toast.success(response.data?.message);
            let userPayload = response.data.data.user;
            dispatch({
                payload: {
                    ...userPayload,
    ...
    useEffect(() => {
        let mails = [];
        if(userx.member?.mails) {
            mails = userx.member.mails.map(mail => mail.email);
        }
        let phones = [];
        if(userx.member?.phones) {
            phones = userx.member.phones.map(phone => phone.phone);
        }
        if(userx.member?.interests !== undefined) {
            let new_list_interests = interests.map(interest => {
            let isSelected = false;
            let ids = [];
            ids = userx.member.interests.map(inter => inter.id);
        ...
        setValue('name', userx.name);
        setValue('surname', userx.surname);
        setValue('second_surname', userx.second_surname);
        setValue('username', userx.username);
        setValue('password', userx.password);
        setValue('delegation', userx.member?.delegation.uuid);
        setValue('email_institutional', userx.member?.email_institutional);
        setValue('email_personal', userx.member?.email_personal);
        setValue('mails', mails.join(','));
        setValue('gender', userx.member?.gender);
        setValue('ci', userx.member?.ci);
        setValue('fullname_workplace', userx.member?.fullname_workplace);
        setValue('abbreviated_name_workplace', userx.member?.abbreviated_name_workplace);
        setValue('phone_workplace', userx.member?.phone_workplace);
        setValue('cellphone_workplace', userx.member?.cellphone_workplace);
        setValue('postal_address_workplace', userx.member?.postal_address_workplace);
        setValue('home_address', userx.member?.home_address);
        setValue('home_phone', userx.member?.home_phone);
        setValue('home_cellphone', userx.member?.home_cellphone);
        setValue('phones', phones.join(','));
        setValue('university_degree', userx.member?.university_degree);
        setValue('university_course_name', userx.member?.university_course_name);
        setValue('university_study_center', userx.member?.university_study_center);
        setValue('university_graduate_year', userx.member?.university_graduate_year);
        setValue('university_city', userx.member?.university_city);
        setValue('university_country', userx.member?.university_country);
        setValue('master_title_achieved', userx.member?.master_title_achieved);
        setValue('master_year', userx.member?.master_year == 'null' ? '' : userx.member?.master_year);
        setValue('master_study_center', userx.member?.master_study_center);
        setValue('master_city', userx.member?.master_city);
        setValue('master_country', userx.member?.master_country);
        setValue('doctoral_degree_achieved', userx.member?.doctoral_degree_achieved);
        setValue('doctoral_year', userx.member?.doctoral_year == 'null' ? '' : userx.member?.doctoral_year);
        setValue('doctoral_study_center', userx.member?.doctoral_study_center);
        setValue('doctoral_city', userx.member?.doctoral_city);
        setValue('doctoral_country', userx.member?.doctoral_country);
        setValue('docent_category', userx.member?.docent_category);
        setValue('docent_year', userx.member?.docent_year);
        setValue('scientific_category', userx.member?.scientific_category);
        setValue('scientific_year', userx.member?.scientific_year == 'null' ? '' : userx.member?.scientific_year);
        setValue('others_interest', userx.member?.others_interest);
    }, [userx]);
